...
const ScenariosOverview = () => {
  ...
  const [pendingScenario, setPendingScenario] = useState<ScenarioSchema | null>(null);

  // Modal state for selling market orders
  const [sellMarketModalOpen, setSellMarketModalOpen] = useState(false);
  const [sellMarketScenario, setSellMarketScenario] = useState<ScenarioSchema | null>(null);
  ...
  const columns = useMemo<MRT_ColumnDef<ScenarioSchema>[]>(
    () =>
      getScenarioColumns({
        ...
        updatingScenarioId,
        onDeleteScenario,
        onCancelTradesByScenario,
        onOpenPendingModal: handleOpenPendingModal,
        onSellMarketModal: handleOpenSellMarketModal,
      }),
    [isUpdatingScenario],
  );
  ...
  return (
    <>
      ...
      >
        <div>
          Are you sure you want to remove all pending broker orders for this scenario?
        </div>
